...
function Guide({ destination, setDestination, camera, setCamera }: { destination: string; setDestination: (value: string) => void; camera: boolean; setCamera: (value: boolean) => void }) {
  ...
  useEffect(() => {
    ...
    if (!cameraRequest) {
      setCameraError('Camera access is unavailable. You can still follow the coordinate route preview.')
      return
    }
    void cameraRequest.then(nextStream => {
      stream = nextStream
      if (videoRef.current) videoRef.current.srcObject = nextStream
    }).catch(() => setCameraError('Camera access is unavailable. You can still follow the coordinate route preview.'))
